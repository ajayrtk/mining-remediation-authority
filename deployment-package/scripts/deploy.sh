...
# Includes: Infrastructure, Processor Container, Frontend, and Configuration
...
if ! ./scripts/configure_aws.sh; then
    echo -e "${RED}ERROR: AWS configuration failed${NC}"
    echo -e "${YELLOW}Please check your .env file and AWS credentials${NC}"
    exit 1
fi
...
# AWS_ACCOUNT already validated by configure_aws.sh
AWS_ACCOUNT=$(aws sts get-caller-identity --query Account --output text)
# Use AWS_DEFAULT_REGION from .env, fallback to terraform.tfvars
AWS_REGION=${AWS_DEFAULT_REGION:-$(grep -E '^\s*aws_region\s*=' infra/terraform.tfvars | sed 's/#.*//' | sed 's/.*=[[:space:]]*"\([^"]*\)".*/\1/' || echo "eu-west-2")}
...
if [ -z "$PROCESSOR_REPO_PATH" ]; then
    ...
    echo -e "${YELLOW}PROCESSOR_REPO_PATH=/path/to/mra-mine-plans-ds${NC}"
    exit 1
fi

echo -e "${GREEN}Processor repository: ${PROCESSOR_REPO_PATH}${NC}\n"
...
# Step 4: Build and push processor container
echo -e "${BLUE}[4/8]${NC} ${YELLOW}Building and deploying processor container...${NC}"

# Processor repository path loaded from .env
echo "  • Using processor repository: $PROCESSOR_REPO_PATH"

if [ -d "$PROCESSOR_REPO_PATH" ]; then
    echo "  • Processor repository found at: $PROCESSOR_REPO_PATH"

    # Save current directory
    CURRENT_DIR=$(pwd)

    # Navigate to processor repository
    cd "$PROCESSOR_REPO_PATH"

    # Check if build script exists
    if [ -f "build_and_push.sh" ]; then
        echo "  • Running processor build script..."

        # Update ECR repository name in build script to match environment
        # This ensures processor builds to correct environment (prod/staging)
        TEMP_BUILD_SCRIPT=$(mktemp)
        sed "s/mra-mines-processor-staging/mra-mines-processor-${ENVIRONMENT}/g" build_and_push.sh > "$TEMP_BUILD_SCRIPT"
        sed -i.bak "s/ECS_CLUSTER=\"mra-mines-cluster-staging\"/ECS_CLUSTER=\"${PROJECT_NAME}-cluster-${ENVIRONMENT}\"/g" "$TEMP_BUILD_SCRIPT"
        sed -i.bak "s/ECS_SERVICE=\"mra-mines-processor-staging\"/ECS_SERVICE=\"${PROJECT_NAME}-processor-${ENVIRONMENT}\"/g" "$TEMP_BUILD_SCRIPT"
        chmod +x "$TEMP_BUILD_SCRIPT"

        # Run processor build
        if "$TEMP_BUILD_SCRIPT"; then
            echo -e "${GREEN}✓${NC} Processor container built and pushed to ECR"
            rm -f "$TEMP_BUILD_SCRIPT" "${TEMP_BUILD_SCRIPT}.bak"
        else
            echo -e "${RED}ERROR: Processor build failed${NC}"
            rm -f "$TEMP_BUILD_SCRIPT" "${TEMP_BUILD_SCRIPT}.bak"
            cd "$CURRENT_DIR"
            exit 1
        fi
    else
        echo -e "${RED}ERROR: build_and_push.sh not found in processor repository${NC}"
        cd "$CURRENT_DIR"
        exit 1
    fi
    ...
    echo -e "${YELLOW}⚠ Processor repository not found at: $PROCESSOR_REPO_PATH${NC}"
    echo -e "${YELLOW}⚠ Skipping processor build - ensure processor image exists in ECR${NC}"
    echo -e "${YELLOW}  You may need to build it manually later.${NC}"
fi

echo ""

# Step 5: Build and push frontend
echo -e "${BLUE}[5/8]${NC} ${YELLOW}Building and deploying frontend application...${NC}"
